...
## 9. Contas a Pagar (`AccountsPayable`, Fase 1 do roadmap Tiny ERP, 28/07/2026)

Espelho do lado "a pagar" de `ReceivableRecord` — mesmo schema Postgres (`financial_intelligence`), decisão explícita de não abrir um schema novo (ver comentário no `prisma/schema.prisma`, acima do model): é um registro financeiro como `ReceivableRecord`, não específico de compra. Ordem de Compra (próxima etapa da Fase 1) vai CRIAR linhas aqui via evento/porta quando existir — uma conta a pagar avulsa (aluguel de terceiro, imposto, frete) não precisa nunca ter passado por uma Ordem de Compra.

- **Fornecedor**: `supplierId` é referência SOLTA (não FK) para `catalog.Supplier` — mesmo padrão de `ReceivableRecord.skuCode`. `AccountsPayableService` valida ownership do tenant contra `SUPPLIER_REPOSITORY` (porta exportada pelo `CatalogModule` especificamente para este consumo) antes de gravar. Opcional — nem toda conta tem fornecedor cadastrado.
- **Baixa**: único caminho é `AccountsPayableService.markPaid()` (mesmo racional de `ReceivableReconciliationService.reconcile()` para `ReceivableRecord` — nunca duas rotas mudando o mesmo status). Idempotente (chamar de novo numa conta já `PAID` é um no-op) e emite `ACCOUNTS_PAYABLE_EVENTS.PAID`.
- **Baixa com ajuste (Quick Win 4, benchmark Bling, `docs/bling-erp-benchmark-analysis.md`)**: `markPaid()` aceita `interestAmount`/`discountAmount`/`feeAmount` opcionais (espelho de `ContasBaixarContaDTO.juros/desconto/tarifa` do Bling) — o valor efetivamente baixado (`paidAmount`) pode diferir do valor de face (`amount`), que é o normal em pagamento com atraso (juros), negociação (desconto) ou taxa da forma de pagamento (tarifa). `resolvePaidAmount()` (função pura, `domain/accounts-payable.entity.ts`) calcula `amount + juros - desconto + tarifa`; `isValidSettlementAdjustments()` rejeita ajuste negativo e o service rejeita `paidAmount` resultante <= 0. `amount` original nunca é sobrescrito — `paidAmount`/`interestAmount`/`discountAmount`/`feeAmount` são colunas novas e nulas até a baixa acontecer.
- **Status derivado, não gravado**: `OVERDUE` nunca é persistido — é calculado on-the-fly por `resolveEffectiveStatus()` toda vez que a conta é lida (list/findOne), comparando `dueDate` com `now`. Evita precisar de um cron que varre e atualiza linhas vencidas (vai além do que `ReceivableStatus.OVERDUE` documenta, que é gravado por um job futuro ainda não implementado).
- **Parcelamento**: `occurrence = PARCELADA` é a única ocorrência que efetivamente gera múltiplos registros — `buildInstallmentPlan()` (função pura, `domain/accounts-payable.entity.ts`) rateia o valor em N parcelas (resto de centavos na última) e distribui `dueDate` mensalmente a partir da primeira, ligadas por `installmentGroupId`. `SEMANAL`/`MENSAL`/`TRIMESTRAL` hoje só rotulam a intenção de recorrência de uma única conta — gerar automaticamente a próxima ocorrência é um limite MVP consciente (mesmo limite que `FixedExpense.dueDay` já documenta).
- **Tags**: `TaggableEntityType` ganhou `ACCOUNTS_PAYABLE` (o sistema de tags genérico, polimórfico desde a Fase 0, já previa isso) — cobre `POST /contas-pagar/{id}/marcadores` do Tiny.
...
- DRE (seção 7) não inclui `FixedExpense` nem `AccountsPayable` (seção 9) na margem — é margem de contribuição por pedido/canal, não lucro líquido do período (ratear despesa fixa/variável por canal exigiria a mesma premissa de volume ainda não decidida na seção 4).
- DRE carrega todos os pedidos do período em memória para agregar (aceitável para volume de MVP, ver seção 7).
- `AccountsPayable` com `occurrence` SEMANAL/MENSAL/TRIMESTRAL não gera automaticamente a próxima ocorrência — só PARCELADA gera múltiplas linhas hoje (ver seção 9).
- Testes: `generic-settlement-parser.spec.ts` (parsing JSON/CSV, casos de erro), `receivable-reconciliation.service.spec.ts` (match, sem match, idempotência), `packaging.service.spec.ts` (evento emitido só quando `costPrice` muda de fato), `packaging-cost-change.listener.spec.ts` (fan-out para SKUs vinculados, falha em um não trava os demais), testes de DRE listados na seção 7, `accounts-payable.entity.spec.ts` (rateio de parcelas, status derivado) e `accounts-payable.service.spec.ts` (baixa idempotente, cancelamento, validação de fornecedor) para a seção 9.
