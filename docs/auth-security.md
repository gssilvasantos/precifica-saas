...
- ~~Se a base de teste (`Test Partner_id`/`Test API Partner Key`) usa a mesma `https://partner.shopeemobile.com` ou um subdomínio de sandbox dedicado~~ **CONFIRMADO em produção (27/07/2026):** NÃO usa a mesma base — o primeiro handshake real (clicar em "Conectar com Shopee" contra `https://partner.shopeemobile.com`) devolveu `{"error":"invalid_partner_id","message":"Invalid partner_id, please have a check."}`. A base correta para credenciais de teste é `https://partner.test-stable.shopeemobile.com` (confirmado via busca — é o domínio oficial do "Shopee Open Platform Admin" de sandbox). `SHOPEE_BASE_URL` foi atualizada em `.env.example`/produção para esse valor.
- Se a técnica de embutir `state` no `redirect` sobrevive ao redirect real da Shopee — ainda não confirmado (o handshake anterior falhou antes de chegar nessa etapa).
- O formato exato da resposta de erro de negócio (`ShopeeOAuthTokenResponse.error`/`message`) em cenários de `code` inválido/expirado — parcialmente confirmado: o formato de erro de `auth_partner` (`{error, message, request_id}`) bate com o assumido; o de `/auth/token/get` ainda não foi exercitado.
