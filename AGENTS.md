...
Instruções detalhadas moram em `.Codex/skills/product-engineering-studio/`,
`.Codex/rules/` e `docs/` — este arquivo é só o mapa.
...
```bash
bash .Codex/skills/product-engineering-studio/scripts/validate-project.sh
```
...
Subagentes disponíveis em `.Codex/agents/`: `product-architect`, `software-architect`,
`frontend-architect`, `backend-architect`, `database-architect`, `security-reviewer`,
`qa-reviewer`, `devops-reviewer`.

Documentação do sistema: `docs/architecture/Codex-engineering-system.md`.
...
> `apps/web/AGENTS.md` e `apps/api/AGENTS.md`.
