...
**Importante — dado já gravado errado não se autocorrige sozinho**: a sincronização incremental (janela de 7 dias, depois da primeira) nunca re-toca um pedido mais antigo que isso — então os 316 pedidos já salvos com status errado ficam errados para sempre a menos que sejam ressincronizados do zero. Script de manutenção único `apps/api/clear-mercado-livre-orders.ts` apaga os pedidos reais do Mercado Livre do tenant demo (seguro — o Mercado Livre é a fonte da verdade, nada se perde) para forçar o próximo ciclo do cron a tratar como primeira sincronização de novo, já com a lógica corrigida.
