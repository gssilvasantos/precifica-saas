...
## Módulo de Ads Multicanal — Fase 0 (acesso às APIs) — **STANDBY LIBERADO (24/07/2026)**

**Pausada em 16/07/2026 por decisão de negócio, não por bloqueio técnico** — condicionada a duas frentes: (1) RLS multi-tenant realmente aplicado em produção (prioridade #1 do usuário na época) e (2) layout/UI maduro o suficiente para uma demonstração visual completa dos fluxos principais, já que pelo menos Shopee exige "produto vivo publicamente acessível" + "conta de teste/trial" e a Amazon exige uma auditoria de segurança formal (Security Onboarding Review) igualmente dependente de um produto demonstrável — ver `docs/marketplace-ads-api-access-plan.md`.

**Ambas as condições foram cumpridas:** RLS fechado e validado em produção (ver seção "Row-Level Security" abaixo) e o Sprint de Layout/UI concluído (Blocos 1–3: Dashboard de Ads + Demo Mode, Governança de MAP + Promoções, Configurações Fiscais — ver seções correspondentes). Mercado Livre nunca dependeu desta pausa (já resolvido desde o Sprint 22 — app OAuth2 já registrado, só falta pedir o escopo `advertising/product_ads` a mais).

**Próximo passo:** executar o checklist de `docs/marketplace-ads-api-access-plan.md` (seção 7) — abrir os 4 processos de acesso (TikTok → Shopee → Amazon → Magalu, ordem sugerida) em paralelo, sem esperar nenhuma aprovação para seguir o desenvolvimento do módulo em si. É trabalho majoritariamente externo/comercial (contas, documentos, contato com times de negócio das plataformas), não código — a Claude preparou materiais de apoio (política de privacidade, textos de cadastro) para acelerar essa etapa.
...
## Row-Level Security (RLS) — completa, aplicada e validada em produção
...
- **`DATABASE_URL` do Render trocada para `app_runtime` — e um segundo bug real encontrado no smoke test.** Depois de trocar a env var em produção, o login com a conta demo passou a devolver 401 mesmo com usuário/hash confirmados corretos no banco. Causa: `TenantContextInterceptor` chamava `.subscribe()` no Observable **fora** da janela do `AsyncLocalStorage.run()` — anti-padrão documentado no próprio repositório do NestJS ([nestjs/nest#15317](https://github.com/nestjs/nest/issues/15317)) que, sob concorrência, podia deixar o contexto de tenant de uma request vazar para outra em vez de abrir o bypass esperado nas rotas públicas (login/signup). Corrigido movendo o `.subscribe()` para dentro do callback. Ver seção 3.4.2 (nova) de `docs/row-level-security-architecture.md` para o racional completo, incluindo por que isso também é relevante para requests autenticadas, não só o login.
- **Validado em produção.** Depois do fix + redeploy: login com a conta demo, dashboard e listagem de produtos confirmados funcionando normalmente em `kyneti.com.br` com `app_runtime` (RLS realmente ativa) em uso. A prioridade #1 do usuário está fechada — Ads Fase 0 e novos canais saem do standby. **Pendente, não-bloqueante**: as 2 suítes E2E do Pick & Pack contra o banco com RLS ativa, e medir latência antes/depois nos endpoints de maior volume (sync de pedidos, DRE).
...
## Integrações — telas de conexão Nuvemshop e Olist (24/07/2026)

Complemento da tela `/integracoes`, que até aqui só tinha UI real para Mercado Livre (OAuth2) — Nuvemshop e Olist já tinham backend completo (`nuvemshop-connection.controller.ts`, `olist-connection.controller.ts`: status/connect/disconnect/sync-now) mas eram API-only, sem tela. Motivo imediato: critério 2 da revisão de parceiro da **Shopee Open Platform** ("produto ativo com seção de Integrações demonstrando integrações ativas de Integrador/ERP/HUB/TMS") exige uma URL de admin que mostre isso de verdade — ver `docs/marketplace-ads-api-access-plan.md`.

- **`features/erp-connections/api.ts`** (novo) — tipos + chamadas para os 4 endpoints de cada canal (status/connect/disconnect/sync-now), espelhando `NuvemshopConnectionStatus`/`OlistConnectionStatus` do backend, mesmo racional de `features/marketplace-connections/api.ts`.
- **`NuvemshopConnectionCard.tsx`** e **`OlistConnectionCard.tsx`** (novo, `features/erp-connections/components/`) — formulário de credencial (storeId+token / apiToken, já que nenhum dos dois usa OAuth2 completo — token estático de "app privado"/API V2), status conectado/desconectado, botão de sincronizar agora, mensagens de erro extraídas do corpo da exceção do NestJS (`lib/extract-error-message.ts`, novo helper reaproveitável).
- **`IntegracoesPage.tsx`** — o card placeholder "tracejado" da Nuvemshop (que só linkava para o README) foi substituído pelos dois cards reais; comentário de topo do arquivo atualizado.
- **Nota sobre a Olist especificamente**: diferente de Mercado Livre/Nuvemshop (canais de venda), a Olist é a integração de ERP — fonte única da verdade do catálogo, o Kyneti só lê (nunca escreve de volta), populando SKU/nome/preço/estoque automaticamente em vez do usuário cadastrar produto na mão.
...
**Segunda causa raiz, encontrada depois do deploy da correção acima**: mesmo com a janela corrigida, zero pedidos continuavam aparecendo. Motivo: `prisma/seed.ts` nunca criou uma linha de `ProviderSyncSchedule` com `capability: 'ORDERS'` — nem para `MERCADO_LIVRE_ORDERS`, nem para `NUVEMSHOP_ORDERS` — desde que o hub de Pedidos existe (Etapa 16). `OrdersSyncSchedulerJob.findDue()` filtra exatamente por esse capability; sem a linha, o cron de 10 min sempre encontra zero schedules vencidos e retorna sem fazer nada, sem logar aviso nenhum (`if (ordersDue.length === 0) return;`, silencioso por design). Ou seja: o sync de pedidos JAMAIS rodou automaticamente para nenhum canal, desde sempre — só passou despercebido até agora porque o Demo Mode (dados semeados manualmente) nunca dependeu dele. Corrigido adicionando as duas linhas faltantes no seed (`intervalMinutes: 10`, mesma cadência do `@Cron`). Precisa rodar `npm run seed` (ou `prisma:seed`) apontando para o banco de produção depois do deploy — não é automático, o seed não roda sozinho no boot.

**Terceira causa raiz, confirmada em log ao vivo depois das duas correções acima**: com o schedule criado e a janela de backfill correta, o log mostrou exatamente o esperado — `[OrderSyncOrchestrator] Primeira sincronização de MERCADO_LIVRE_ORDERS ... usando janela de backfill de 730 dias` — mas 6 segundos depois: `ERROR ... Sync de pedidos de MERCADO_LIVRE_ORDERS falhou: Mercado Livre /orders/search retornou HTTP 429 (offset 2100)`. Causa: `MercadoLivreApiClient` nunca usava o `RateLimiter`/`withRetry` compartilhado (`shared/rate-limiting/`, existente desde a Etapa 17 e já usado por `NuvemshopApiClient`) — a paginação de `/orders/search` disparava ~42 requisições em poucos segundos sem nenhum throttling, e o Mercado Livre bloqueou com 429. Sem retry, o erro derrubava a sincronização inteira, descartando até os pedidos já buscados nas páginas anteriores.

**Correção** (`mercado-livre-api.client.ts`): toda chamada de rede da classe agora passa por um `request()` privado único, que agenda via `RateLimiter` (configurado com `getRateLimitConfig('MERCADO_LIVRE')` — sem entrada específica no `marketplace-rate-limits.ts` ainda, cai no fail-safe conservador de 1 req/s, mesmo aviso de honestidade já documentado lá) e retenta com backoff exponencial especificamente em HTTP 429 via `withRetry`/`isRateLimitError`. Nenhuma mudança de comportamento para os demais status HTTP (404/500/...), que continuam retornando normalmente para cada método decidir.

**Quarta causa raiz, apontada pelo usuário comparando com o painel real do Mercado Livre**: com os 3 bugs acima corrigidos, os pedidos finalmente apareceram (316) — mas TODOS presos em "Preparando envio", incluindo pedidos de mais de um mês atrás, quando o usuário via no painel do Mercado Livre só 36 "em aberto" + 7 "aprovados" (43 pedidos realmente ativos, não 316). Causa: o campo `shipping` devolvido por `/orders/search` é só uma REFERÊNCIA (`{id: <shipment_id>}`), nunca o status de fato — a suposição original em `mercado-livre-order-normalizer.ts` (`shipping.status`) era um aviso de honestidade nunca validado contra payload real, e o primeiro sync revelou que estava errado: `shippingStatus` ficava sempre `undefined`, e `mapMercadoLivreStatus` cai em `PREPARANDO_ENVIO` para todo pedido pago, para sempre.

**Correção**: `MercadoLivreApiClient.fetchShipmentStatus(shipmentId, accessToken)` (novo) consulta o sub-recurso real `GET /shipments/{id}` (mesmo `request()` com rate limit). `mercado-livre-order-normalizer.ts` ganhou `resolvedShippingStatus` como segundo parâmetro de `normalizeMercadoLivreOrder` (tem prioridade sobre o fallback antigo) + `extractShippingId`/`extractOrderStatus` exportados. `MercadoLivreOrderProvider.fetchOrders` agora consulta o envio real ANTES de normalizar, mas só para pedidos com status `paid`/`partially_paid` (`STATUSES_WORTH_CHECKING_SHIPMENT`) — pedido em aberto/cancelado não precisa, economizando chamadas de API num backfill grande. Testes atualizados/adicionados nos 3 arquivos (`mercado-livre-order-normalizer.spec.ts`, `mercado-livre-order.provider.spec.ts`).

**Importante — dado já gravado errado não se autocorrige sozinho**: a sincronização incremental (janela de 7 dias, depois da primeira) nunca re-toca um pedido mais antigo que isso — então os pedidos já salvos com status errado ficam errados para sempre a menos que sejam ressincronizados do zero. Script de manutenção único `apps/api/clear-mercado-livre-orders.ts` apaga os pedidos reais do Mercado Livre do tenant demo (seguro — o Mercado Livre é a fonte da verdade, nada se perde) para forçar o próximo ciclo do cron a tratar como primeira sincronização de novo, já com a lógica corrigida.

**Botão "Sincronizar agora" — generalizado para a aba Pedidos, não um card isolado (sugestão do usuário)**: em vez de um botão dedicado só no card do Mercado Livre em Integrações, `triggerOrderSync` (novo, `features/orders/api.ts`) e o botão em `OrderTable.tsx` funcionam para QUALQUER canal com `providerCode` real em `ORDER_CHANNELS` (`channels.ts`) — hoje Nuvemshop e Mercado Livre. Só aparece com um canal específico selecionado no filtro (nunca em "Todos os canais", que misturaria providers numa única chamada). AVISO explícito na UI ("pode levar minutos") porque o endpoint (`OrdersSyncController.triggerSync`) espera a sincronização terminar antes de responder — com o rate limit conservador do Mercado Livre, isso é literal, não figura de linguagem. Corrigido de quebra um bug cosmético antigo: `MERCADO_LIVRE` estava com `implemented: false` em `channels.ts` (por isso aparecia "(em breve)" no dropdown mesmo já sincronizando pedidos reais desde a Sprint 22).

**Quinta causa raiz — o backfill de 2 anos nunca conseguia terminar no plano free do Render**: depois de rodar `clear-mercado-livre-orders.ts` (apagou os 1010 pedidos com status errado) para forçar um backfill limpo com a correção da quarta causa raiz, o log do Render mostrou ~7 tentativas ao longo de quase 2h (a cada ~10-30 min, batendo com o cron), TODAS logando `Primeira sincronização de MERCADO_LIVRE_ORDERS ... usando janela de backfill de 730 dias` — nunca uma vez "sincronização incremental" — com IDs de instância diferentes a cada tentativa (`pk6nz`, `bbfx2`, `jmffk`, `9vmrk`, `vf2vg`...). Isso só acontece se `hasAnyOrderForChannel` continuar retornando `false` para sempre, ou seja: nenhuma das tentativas persistiu um único pedido sequer. Causa: o plano free do Render hiberna/reinicia a instância por inatividade de tráfego HTTP (aviso visível no próprio painel: "Your free instance will spin down with inactivity") — mas não enxerga um cron rodando em background como "atividade". Com ~1010 pedidos e uma consulta adicional de envio por pedido pago (`GET /shipments/{id}`, rate limit de 1 req/s), o backfill sozinho já levava 15-20+ minutos só de chamadas de rede — tempo mais que suficiente para o Render matar o processo no meio. Como `syncTenant` só persiste os pedidos DEPOIS que `fetchOrders` retorna a lista inteira já buscada e enriquecida (nunca incrementalmente, um a um, durante a busca), uma morte no meio do caminho descarta 100% do progresso daquela tentativa — inclusive chamadas de API já gastas — e a tentativa seguinte recomeça do zero, para sempre, sem nunca convergir.

**Correção**: `BACKFILL_WINDOW_MS` reduzido de 2 anos para **90 dias** (`order-sync-orchestrator.service.ts`) — cobre com folga qualquer pedido ainda operacionalmente relevante (em aberto/aprovado/aguardando envio; o usuário confirmou só 43 pedidos realmente ativos hoje) e reduz o volume de chamadas o suficiente para o backfill terminar dentro de uma única janela de atividade do Render antes de hibernar. Histórico mais antigo que 90 dias não é recuperado automaticamente — limitação aceita, não um bug, dado o custo de manter o Render acordado por 20+ minutos só para isso. De quebra, adicionada uma guarda de execução concorrente (`providersInFlight`, `Set<string>` em memória em `OrderSyncOrchestrator`): se o cron de 10 em 10 min disparar de novo (ou alguém clicar "Sincronizar agora") enquanto uma sincronização do MESMO provider ainda está em andamento nesta instância, o novo disparo é ignorado com um aviso no log em vez de rodar em paralelo e brigar pelo mesmo rate limit — problema real exposto pelos logs, já que várias tentativas ficaram empilhadas sem nunca terminar.

**Se ainda não convergir depois deste fix**: o próximo suspeito é o próprio Render free tier ser incompatível com qualquer sync que precise de mais que alguns minutos contínuos — nesse caso a solução não é mais código, é upgrade de plano (remove a hibernação) ou uma reestruturação maior para persistir pedido por pedido durante a busca em vez de só no final (mudança de contrato do provider, não feita aqui por ainda não ter evidência de que os 90 dias não bastam).

**Sexta causa raiz, encontrada via log de diagnóstico temporário (25/07/2026)**: mesmo depois da janela de 90 dias, `ProviderSyncLog` mostrava `status: SUCCESS, candidatesFound: 0` em TODA tentativa desde a noite anterior — nem sucesso real (pedidos aplicados), nem erro. Adicionado um log temporário em `MercadoLivreApiClient.fetchOrders` (`DEBUG-ML-SEARCH`, sem PII, só contagens/paging) para ver a resposta real da API a cada página. Resultado revelador: o filtro `order.date_last_updated.from` (usado desde sempre, para incremental E backfill) devolveu **4674 pedidos** numa janela de 90 dias — não os ~43 realmente ativos que o usuário via no painel do Mercado Livre. Causa: esse campo reflete QUALQUER toque no pedido (reindexação/atualização interna do Mercado Livre), não pedidos literalmente criados no período — para uma conta com milhares de pedidos históricos, isso captura praticamente todo o histórico como "candidato", não só o que é novo. Como cada pedido pago ainda dispara uma consulta extra de status de envio (`fetchShipmentStatus`, rate limit de 1 req/s), só o enriquecimento desses milhares de falsos positivos levaria bem mais de uma hora — e nada é persistido até o lote inteiro terminar (`OrderSyncOrchestrator.syncTenant` só faz upsert depois que `fetchOrders` retorna por completo). Essa é a causa raiz de verdade por trás de toda a série de tentativas que nunca completavam, incluindas as investigadas nas causas raiz 3ª/5ª acima (o rate limit e a hibernação do Render eram sintomas reais, mas o volume de "falsos candidatos" é que tornava qualquer uma delas fatal).

**Correção**: `FetchContext` (contrato compartilhado, `marketplace-provider.contract.ts`) ganhou `isFirstSync?: boolean`, calculado pelo orquestrador (`hasAnyOrderForChannel`) e repassado a todo provider — campo opcional, outros canais ignoram sem quebrar nada. `MercadoLivreApiClient.fetchOrders` ganhou um 4º parâmetro `dateField: 'date_created' | 'date_last_updated'`. `MercadoLivreOrderProvider.fetchOrders` agora escolhe: **backfill (primeira sincronização) usa `date_created`** — literalmente pedidos criados na janela, o volume real e esperado; **incremental continua em `date_last_updated`** de propósito, porque aí sim queremos pegar um pedido antigo que só mudou de status agora, e a janela curta (7 dias) mantém isso seguro mesmo incluindo pedidos "só tocados". O log temporário `DEBUG-ML-SEARCH` foi removido depois de servir seu propósito de diagnóstico. Testes (`mercado-livre-order.provider.spec.ts`) atualizados para refletir a nova assinatura de `client.fetchOrders`.

**Sétima causa raiz — provavelmente a mais fundamental de todas**: mesmo depois da correção do campo de data, uma nova tentativa (disparada manualmente pelo usuário) continuou presa para sempre — `ProviderSyncLog` mostrando `finishedAt: null` minutos depois, com nenhuma mudança mesmo após esperar. Investigação do `RateLimiter`/`withRetry` não achou bug (ambos eventualmente resolvem ou lançam). A causa real: **nenhuma chamada `fetch()` de `MercadoLivreApiClient` tinha timeout**. O `fetch` nativo do Node não tem timeout implícito — se uma única requisição (busca paginada ou, mais provável dado o volume, uma das centenas de consultas de status de envio por pedido pago) travasse sem a API do Mercado Livre nunca responder OK nem erro, a Promise correspondente ficava pendente para sempre. Como `MercadoLivreOrderProvider.fetchOrders` usa `Promise.all` para as consultas de envio, uma ÚNICA chamada travada travava a sincronização inteira: nunca lançava exceção (então o log nunca recebia `FAILED`) e nunca completava (então nunca recebia `SUCCESS` de verdade) — exatamente o padrão observado em produção há mais de 12 horas, em toda tentativa, independente de todas as outras correções já aplicadas.

**Correção**: `MercadoLivreApiClient.request()` agora passa por um `fetchWithTimeout()` novo, com `AbortController` e timeout de 20 segundos por requisição — abortada a chamada, o erro (`isTimeoutError`, novo predicado em `shared/rate-limiting/with-retry.ts`) é tratado como retryable pelo `withRetry` já existente, igual a um HTTP 429. Isso garante que toda chamada eventualmente resolve OU lança um erro real — nunca mais fica pendente indefinidamente. Efeito prático: agora, mesmo se uma consulta de envio travar, o pior caso é ela contar como falha (capturada pelo try/catch de `fetchShipmentStatus`, que já devolvia `null` em caso de erro) em vez de travar TODO o backfill para sempre.

**Oitava causa raiz — a conta tem volume real muito maior do que o assumido**: com o timeout aplicado, um log de fase temporário (`DEBUG-ML-PHASE`) revelou o número real: a busca por `date_created` nos últimos 90 dias devolveu **4455 pedidos**, dos quais **4244 pagos** (elegíveis para consulta de envio). Isso não é bug — a loja processa esse volume de verdade (mais da metade do histórico de ~8800 pedidos aconteceu só neste trimestre) — mas a 1 requisição/segundo (rate limit conservador do `MercadoLivreApiClient`), só a fase de enriquecimento levaria mais de 1 hora, inviável para uma chamada síncrona (e para o Render free tier).

**Correção, confirmada com o usuário**: `MercadoLivreOrderProvider` ganhou `ENRICHMENT_WINDOW_MS` (30 dias) — a consulta extra de status de envio (`/shipments/{id}`) só é feita para pedidos pagos **criados dentro dessa janela recente** (`extractOrderCreatedAt`, novo em `mercado-livre-order-normalizer.ts`). Pedido pago há mais de 30 dias quase certamente já foi enviado/entregue; gastar uma chamada de API nele a cada backfill não compensa. Pedidos fora da janela de enriquecimento entram com o status derivado só do que a busca já traz (o fallback antigo do normalizador — nunca finge um status confirmado sem consultar a fonte real). **Limitação aceita e documentada, não escondida**: o status de envio de pedidos pagos há mais de 30 dias pode ficar temporariamente impreciso até a próxima vez que aquele pedido específico entrar numa janela de enriquecimento (ex.: se o backfill rodar de novo, ou numa iteração futura que amplie a janela seletivamente). Os logs de diagnóstico temporários (`DEBUG-ML-PHASE`) foram removidos; ficou um log permanente e enxuto (contagem de pedidos buscados/enriquecidos por sync, sem PII). Testes (`mercado-livre-order.provider.spec.ts`) atualizados para usar datas relativas a "agora" (nunca absolutas — evita testes que quebram sozinhos com o tempo) e um novo caso cobrindo o corte de 30 dias.

## Reestruturação do sync ML — persistência resumível, sem depender de plano pago (25-26/07/2026)

Mesmo depois das oito causas raiz acima corrigidas (janela de backfill, filtro de data, timeout de rede, janela de enriquecimento de 30 dias), o sync **ainda não completava**: o log permanente de conclusão (`Sync ML tenant=...`) nunca apareceu no Render, em nenhuma tentativa, confirmando que `MercadoLivreOrderProvider.fetchOrders()` nunca chegava a resolver. Diagnóstico final, explicado diretamente ao usuário sem mais rodadas de "mais uma correção pontual": o problema de fundo não era mais nenhum bug de código — era o **plano free do Render**, que hiberna a instância depois de ~15 minutos sem tráfego HTTP de entrada (não reconhece um cron rodando em segundo plano como "atividade"). Mesmo com o volume reduzido pela janela de 30 dias, a fase de enriquecimento de uma conta de alto volume plausivelmente ultrapassava essa janela — e como o desenho antigo só persistia os pedidos **depois** que `fetchOrders()` retornava a lista inteira já buscada E enriquecida, qualquer morte no meio do caminho descartava 100% do progresso daquela tentativa, para sempre, sem nunca convergir.

O usuário perguntou diretamente se a solução era trocar de hospedagem, migrar para um plano pago, ou algo estrutural — resposta dada: nenhuma plataforma resolve isso sozinha; o desenho correto é gravar cada pedido no banco assim que é processado (não em lote no final) e guardar o progresso do enriquecimento como estado no banco (não um cursor em memória) — isso funciona em **qualquer** hospedagem, inclusive a gratuita, porque cada execução processa só um pedaço pequeno e delimitado, nunca tudo de uma vez. O usuário confirmou essa direção explicitamente, optando por **não** migrar para o plano Render Pro por ora ("quero investir quando o sistema realmente for crescer e gerar renda").

**Mudança de arquitetura (não mais um bug pontual) — duas fases agora separadas:**

1. **Busca rápida (fast path), sem enriquecimento inline.** `MercadoLivreOrderProvider.fetchOrders()` foi radicalmente simplificado: só busca as páginas de `/orders/search` e normaliza, **nunca mais** chama `GET /shipments/{id}` por pedido dentro do próprio fetch. Custo agora é proporcional ao número de PÁGINAS da busca, não ao número de pedidos pagos — sempre rápido, independente do volume histórico da conta. Todo pedido pago entra imediatamente com o status-fallback `PREPARANDO_ENVIO` e é persistido pelo `OrderSyncOrchestrator` no mesmo ciclo de sempre (upsert por pedido, dentro do loop existente) — o vendedor já vê o pedido na worklist na mesma hora, mesmo sem status de envio confirmado ainda.

2. **Enriquecimento assíncrono, resumível e em lotes pequenos.** Novo `MercadoLivreShipmentEnrichmentJob` (`apps/api/src/modules/orders/infrastructure/scheduler/`, roda a cada 5 minutos) processa, por tenant, um lote **fixo e pequeno** (40 pedidos) de pedidos `PREPARANDO_ENVIO` ainda não conferidos — consulta real `/shipments/{id}` (mesmo `MercadoLivreApiClient`, mesmo rate limit de 1 req/s) e re-grava o pedido com o status real (`ENVIADO`/`ENTREGUE`/ainda `PREPARANDO_ENVIO`) via `OrderSyncOrchestrator.upsertAndEmit` (agora público, reaproveitado em vez de duplicar a lógica de resolução de SKU/custo e emissão de eventos). O progresso fica gravado **por pedido, no banco** — nova coluna `Order.shippingStatusCheckedAt` (`DateTime?`, migração `20260726120000_order_shipping_status_checked_at`) + índice composto `(tenantId, channelCode, status, shippingStatusCheckedAt, orderedAt)`. `null` = "ainda não conferido de verdade"; só vira não-nulo quando o status resultante SAI de `PREPARANDO_ENVIO` (confirmação definitiva) — se o Mercado Livre ainda responde "em preparação", o pedido continua null de propósito e volta a ser tentado num ciclo seguinte, em vez de ficar preso depois de uma única checagem. Se não há `shipping.id` no payload (nunca vai ter envio pra checar), marca como conferido igual, pra não ficar pra sempre na fila. Fila ordenada do pedido mais ANTIGO pro mais novo (não o contrário) — garante que pedidos antigos não fiquem pra sempre atrás de pedidos novos que chegam a cada sync.

   Isso é o que torna o enriquecimento resumível de verdade: uma interrupção a qualquer momento (inclusive a hibernação do Render no meio da execução) perde no máximo o lote em andamento — nunca o trabalho já persistido de execuções anteriores. Cada execução do job é, por construção, curta o bastante (~40s de chamadas de rede) para nunca chegar perto do limite de inatividade do Render.

3. **Risco novo identificado e corrigido: regressão de status.** Separar as duas fases criou um risco que não existia antes: como `fetchOrders()` (fast path) nunca mais confirma envio, uma resync incremental comum (a cada poucos minutos, sem nenhuma informação nova de envio) sempre normaliza pedido pago para o fallback `PREPARANDO_ENVIO` — e, sem cuidado, isso reescreveria de volta um pedido que o job de enriquecimento já tinha confirmado como `ENVIADO`/`ENTREGUE`, apagando o progresso. Corrigido com uma nova invariante genérica (não específica do Mercado Livre) em `domain/order-status-guard.ts` — `resolveEffectiveStatus()`, função pura: o status de um pedido nunca REGRIDE para um estágio anterior, exceto virar `CANCELADO` (que pode acontecer a qualquer momento); e `CANCELADO`, uma vez atingido, é terminal e não "descancela" sozinho. Aplicada em `PrismaOrderRepository.upsert()`, no único lugar que já lê o status anterior antes de gravar — nunca duplicada por canal.

**Arquivos principais**: `mercado-livre-order.provider.ts` (simplificado — fast path puro), `mercado-livre-shipment-enrichment.job.ts` (novo), `order-status-guard.ts` (novo, puro), `order-sync-orchestrator.service.ts` (`upsertAndEmit` agora público, aceita `overrides` para `shippingStatusCheckedAt`), `prisma-order.repository.ts` (guarda de regressão + preservação de `shippingStatusCheckedAt` quando o chamador não o informa + novo método `findPendingShipmentEnrichment`), `order-repository.port.ts`, `order.entity.ts` (`shippingStatusCheckedAt`/`rawPayload` agora expostos no domínio, não só na persistência), `marketplace-intelligence.module.ts` (exporta `MercadoLivreApiClient`/`MercadoLivreConnectionService`, consumidos pelo novo job que mora no módulo `orders`, mesma direção de dependência de sempre — nunca o contrário), `orders.module.ts` (registra o novo job), schema Prisma + migração hand-written.

Testes novos/atualizados: `order-status-guard.spec.ts` (novo, cobre avanço/regressão/cancelamento/terminalidade), `mercado-livre-shipment-enrichment.job.spec.ts` (novo, cobre lote vazio, confirmação de envio, "ainda em preparação" não marca como conferido, resposta nula não marca, pedido sem shipping.id, falha de token, falha isolada de um pedido não trava o lote), `mercado-livre-order.provider.spec.ts` (reescrito — fast path nunca chama `fetchShipmentStatus`).

Nota de verificação: mesma limitação de rede/sandbox já documentada nas frentes anteriores (RLS, shadcn/ui) — não foi possível rodar `npx prisma migrate dev`/`tsc --noEmit`/`jest` até o fim neste ambiente; a migração foi escrita à mão seguindo exatamente o diff que o Prisma geraria, e o diff completo (assinaturas de tipo, todos os call sites de `OrderRepository`/`Order`/`OrderUpsertData`, todos os mocks de teste) foi revisado manualmente em substituição. Migração e testes precisam rodar numa máquina/pipeline com rede real antes do deploy (mesmo fluxo de sempre: `npx prisma migrate deploy` como Pre-Deploy Command no Render).

## Bug de produção — contadores das abas de status ignoravam o canal selecionado (26/07/2026)

Encontrado pelo usuário ao comparar os contadores da tela de Pedidos com a Olist: a aba "Todos" não mostrava nenhum contador (só as outras 6 abas de status tinham número), e investigação revelou algo mais sério por trás — `OrderRepository.countByStatus` (porta, serviço, controller e implementação Prisma) nunca aceitou `channelCode`. Resultado: os contadores das abas ("Preparando envio (602)" etc.) sempre somavam pedidos de **todos os canais**, mesmo com "Mercado Livre" selecionado no dropdown — inconsistente com a tabela de pedidos abaixo, que já filtrava corretamente por `findWithFilters`.

**Correção**: `channelCode` adicionado como parâmetro opcional em toda a cadeia (`order-repository.port.ts` → `prisma-order.repository.ts` → `orders.service.ts` → `orders.controller.ts` → `features/orders/api.ts` → `OrderTable.tsx`), ausente = todos os canais (mesmo comportamento de antes, agora explícito em vez de acidental). De quebra, a aba "Todos" ganhou seu contador (soma dos 6 status), consistente com as demais.
...
**Atualização (29/07/2026) — retrofit completo, as 10 telas restantes:** Pedidos (`OrdersPage`/`OrderTable`, incluindo a nova `Pagination` numerada), Ads (`AdsPage`/`AdsCampaignCard`/`AdsSuggestionCard`), Catálogo (`CatalogPage`), Governança MAP (`MapGovernancePage`/`MapBulkImportPanel`/`ProductAuditTrail`), Promoções (`PromotionsPage`/`PromotionCampaignDetailPage`/`MarginPreviewSimulator`/`EnrollmentTable`/`CreateCampaignPanel`), Financeiro (`FinanceiroPage`, DRE por pedido), Abastecimento (`AbastecimentoPage`), Conferência (`ConferenciaPage`/`ConferenciaDetalhePage` — a mais sensível, câmera/MediaRecorder/upload de chunks: só classes visuais foram tocadas, a lógica de captura de vídeo/bipagem ficou intacta), Integrações (`IntegracoesPage` + os 3 cards de conexão: Mercado Livre inline, `ShopeeConnectionCard`, `NuvemshopConnectionCard`, `OlistConnectionCard`, `ConnectionStatusBadge` compartilhado) e Configurações Fiscais (`TaxProfilesSection`/`TaxProfileForm`/`DefaultMarginsForm`/`FinancialPolicyForm`). Mesmo padrão da fundação: tokens semânticos (`text-foreground`/`text-muted-foreground`/`bg-muted`/`border-border`/`bg-card`) no lugar de `ink-*`/`canvas`/`surface` hardcoded, `Card`/`Badge`/`Button`/`Skeleton` como primitivos de base. De quebra, dois badges de status "neutro" (`EM_ABERTO` em `status-meta.ts`, `SEM_DADO` em `margin-status.ts`, `DRAFT`/`ENDED` em `CampaignStatusBadge`) migraram de `bg-ink-300/40` fixo para `bg-muted`, ganhando suporte a Dark Mode que não tinham antes. Verificação: `npx tsc --noEmit` do frontend rodado pelo usuário três vezes ao longo do retrofit (após Pedidos/Ads/Catálogo/Governança MAP, após Promoções/Financeiro/Abastecimento/Conferência, e final após Integrações/Configurações Fiscais) — zero erros nas três rodadas.

Retrofit visual das 13 telas do frontend: **completo** (Sidebar, AppLayout, Dashboard nesta seção; as 10 restantes na atualização acima).

## Integração Shopee Open Platform — camada de conexão (27/07/2026)

Usuário criou um app "Kyneti" no Shopee Open Platform (categoria "ERP System" — a mais ampla das disponíveis, coerente com o caráter multi-capacidade do Kyneti), recebeu credenciais de teste (Test Partner_id/Test API Partner Key) e um "Test Redirect URL Domain" já registrado (`https://api.kyneti.com.br`, mesmo domínio já usado pelo callback do Mercado Livre), e pediu para construir a integração com essas credenciais.

**Escopo desta sprint, deliberadamente igual ao do Sprint 21→22 do Mercado Livre:** só a camada de conexão (autorização por navegador, callback, renovação automática de token, desconexão) — nenhum `ShopeeOrderProvider`/endpoint de pedidos/produtos foi implementado ainda. Mesma sequência de antes: primeiro provar que a autenticação funciona de ponta a ponta contra a API real, só depois construir o que consome ela.

**Diferença estrutural para o Mercado Livre — mecanismo de auth:** o Mercado Livre usa OAuth2 clássico (RFC 6749, `client_id`/`client_secret`/`grant_type`); a Shopee assina **toda** chamada com HMAC-SHA256 (`sign = HMAC-SHA256(partner_id + path + timestamp [+ access_token + shop_id], partner_key)`), sem conceito de Bearer token simples. `ShopeeConnectionService` implementa o mesmo contrato `AuthStrategy` já usado pelo Mercado Livre, mas com `type: 'API_KEY_HMAC'` em vez de `'OAUTH2'` — o contrato já previa essa variação desde a Etapa 4, nunca exercitada até agora.

**Peças construídas (mesma árvore de camadas do Mercado Livre, dentro de `marketplace-intelligence` — é aqui que o módulo dono da futura integração de pedidos/preços da Shopee vai morar, mesmo racional de propriedade usado em toda a plataforma):**
- `prisma/schema.prisma` — model `ShopeeConnection` (tenantId, shopId, accessTokenEnc/refreshTokenEnc, expiresAt, isActive, lastRefreshedAt) + migração hand-written (`20260727120000_shopee_connection`).
- `infrastructure/providers/shopee/shopee-api-client.ts` — `ShopeeApiClient`: assinatura HMAC, `buildAuthPartnerUrl` (monta a URL de `/api/v2/shop/auth_partner`), `exchangeCodeForToken`/`refreshAccessToken` (`/api/v2/auth/token/get` e `/api/v2/auth/access_token/get`), mesmo RateLimiter/timeout/retry do `MercadoLivreApiClient`.
- `application/ports/shopee-connection-repository.port.ts` + `infrastructure/prisma-shopee-connection.repository.ts` — mesmo padrão de porta+adapter Prisma do Mercado Livre.
- `application/shopee-connection.service.ts` — `ShopeeConnectionService` (`AuthStrategy`): `buildAuthorizationUrl`/`handleCallback`/`getValidAccessToken`/`getShopId`/`listActiveTenantIds`/`disconnect`/`getStatus`, com o mesmo mecanismo de `state` criptografado (`CredentialEncryptionService`) do Mercado Livre — só que aqui o `state` é embutido manualmente no `redirect` passado à Shopee, porque a Shopee não tem um parâmetro `state` nativo como o Mercado Livre (ver docs/auth-security.md, seção 9, para o racional completo e os avisos de honestidade).
- `interface/controllers/shopee-connection.controller.ts` + `shopee-callback-query.dto.ts` — endpoints `GET status`, `GET authorize` (ADMIN), `GET callback` (público, protegido pelo `state`), `DELETE connect` (desconectar, ADMIN), `POST test-connection` (handshake de diagnóstico, ADMIN — ver abaixo) — mesma estrutura de `MercadoLivreConnectionController`.
- `application/shopee-handshake.service.ts` — `ShopeeHandshakeService`, mesmo racional de `MercadoLivreHandshakeService`: diagnóstico read-only (status → renovação automática → chamada real), nunca escreve pedido/produto. Como ainda não existe nenhum endpoint de pedidos da Shopee, a "prova de vida" usa `GET /api/v2/shop/get_shop_info` (novo método `ShopeeApiClient.fetchShopInfo`, assinatura shop-level com `access_token`+`shop_id`) em vez de `/orders/search` — o endpoint autenticado mais leve disponível.
- `marketplace-intelligence.module.ts` — `ShopeeApiClient`/`ShopeeConnectionService`/`ShopeeHandshakeService`/`PrismaShopeeConnectionRepository`/`ShopeeConnectionController` registrados; nada exportado ainda (nenhum outro módulo consome a Shopee por enquanto).
- `.env.example` — `SHOPEE_PARTNER_ID`/`SHOPEE_PARTNER_KEY`/`SHOPEE_REDIRECT_URI`/`SHOPEE_BASE_URL` (a real Partner Key colada pelo usuário na conversa NUNCA foi escrita em nenhum arquivo — só placeholders vazios, mesma disciplina do Mercado Livre).
- `shared/rate-limiting/marketplace-rate-limits.ts` — entrada `SHOPEE` explícita (1 req/s, igual ao `DEFAULT_RATE_LIMIT`, documentada como decisão consciente e não uma omissão).

**Atualização (27/07/2026) — handshake real confirmado de ponta a ponta:** o usuário completou o fluxo completo contra a Shopee de verdade (sandbox) e clicou em "Testar conexão" na tela de Integrações, com sucesso: `Conexão testada com sucesso. Loja: OpenSANDBOX112597a56484aa076d7. Status da loja: NORMAL.` Isso confirma os três pontos que o aviso de honestidade original desta seção deixava em aberto: (1) a base de teste NÃO é `https://partner.shopeemobile.com` nem a URL de blog de terceiro `https://partner.test-stable.shopeemobile.com` (essa segunda aceitava o partner_id mas rejeitava toda assinatura com "Wrong sign") — a URL real, só encontrada via a "Ferramenta de teste de API" dentro do próprio Shopee Open Platform Console, é `https://openplatform.sandbox.test-stable.shopee.sg`, já configurada em `SHOPEE_BASE_URL`; (2) a técnica de embutir `state` no `redirect` sobreviveu ao redirect real; (3) o handshake de diagnóstico (`get_shop_info`) confirmou o formato de resposta assumido. Detalhe completo da investigação em docs/auth-security.md, seção 9.

Testes: `shopee-api-client.spec.ts` (assinatura HMAC bate com a fórmula documentada, override de `SHOPEE_BASE_URL`, partner_key nunca vaza na URL), `shopee-connection.service.spec.ts` (mesma cobertura de testes do `mercado-livre-connection.service.spec.ts` — autorização não vaza tenantId, callback valida/rejeita state, renovação automática, disconnect/getStatus/listActiveTenantIds), `shopee-handshake.service.spec.ts` (sem conexão ativa não chama a API, sucesso reporta nome/status da loja, token renovado durante o teste, falha emite alerta ERROR sem lançar).

**Frontend — card de conexão na tela de Integrações (mesma sessão):** `features/marketplace-connections/api.ts` ganhou `fetchShopeeStatus`/`fetchShopeeAuthorizeUrl`/`disconnectShopee`/`testShopeeConnection`; `features/marketplace-connections/components/ShopeeConnectionCard.tsx` (novo) segue o mesmo padrão visual do card inline do Mercado Livre em `IntegracoesPage.tsx` — autorizar (redirect completo do navegador)/testar conexão/desconectar — mas componentizado à parte, mesmo racional de `NuvemshopConnectionCard`/`OlistConnectionCard`. `IntegracoesPage.tsx` também generalizou a leitura do callback (`conectado=<canal>`/`erro=<canal>`, antes hardcoded para "Mercado Livre") para funcionar com qualquer canal que devolva esse padrão, hoje Mercado Livre e Shopee.

**Cadastro TikTok Shop Partner Center (27/07/2026, em paralelo):** usuário também iniciou o cadastro como Parceiro (TSP) no TikTok Shop, categoria "Operação e gerenciamento de loja" (a mais próxima de um ERP third-party dentro das opções do TikTok Shop) — usando o mesmo CNPJ da loja Rita Mazzei Beauty (empresário individual, atividade de estética, sem CNAE de tecnologia — ponto assinalado ao usuário como possível questionamento na revisão, mas não bloqueante). Formulário preenchido e enviado, incluindo uma apresentação institucional do Kyneti (PDF gerado nesta sessão) e comprovação de "Content capability" via print de uma LIVE real de 51 minutos na loja. Em análise do lado do TikTok — sem ação nossa pendente até a resposta deles.

## Shopee ORDERS — sincronização real de pedidos (27/07/2026)

Terceiro `OrderCapableProvider` do hub de pedidos (depois de Nuvemshop e Mercado Livre), construído logo depois de o handshake de conexão da Shopee ser confirmado em produção (ver seção anterior) — mesma sequência de maturidade do Mercado Livre: conexão primeiro, pedidos depois.

**Peças construídas, dentro de `marketplace-intelligence/infrastructure/providers/shopee/`:**
- `shopee-api-client.ts` — dois métodos novos: `fetchOrderList` (`/api/v2/order/get_order_list`, fatia a janela pedida em blocos de até 15 dias — limite documentado da Shopee — e pagina por cursor dentro de cada bloco) e `fetchOrderDetail` (`/api/v2/order/get_order_detail`, em lotes de até 50 `order_sn` — outro limite documentado). Nenhuma dessas fatias/lotes vaza para fora do client.
- `shopee-order-status.mapper.ts` — `mapShopeeOrderStatus`, função pura (UNPAID/READY_TO_SHIP/PROCESSED/SHIPPED/COMPLETED/TO_RETURN/CANCELLED/IN_CANCEL → `UnifiedOrderStatus`), fallback conservador (`EM_ABERTO`) para status desconhecido/futuro.
- `shopee-order-normalizer.ts` — `normalizeShopeeOrder`, mesmo racional de `normalizeMercadoLivreOrder`: função pura, `feeAmount` deliberadamente `0` (comissão real da Shopee só sai do endpoint de escrow, não implementado ainda — nunca inventamos uma taxa), nunca fabrica `shippedAt`/`deliveredAt`/`cancelledAt` sem dado real.
- `shopee-order.provider.ts` — `ShopeeOrderProvider` (`OrderCapableProvider`+`AuthenticatedProvider`, capability `ORDERS`, código `SHOPEE_ORDERS`): reaproveita `ShopeeConnectionService.getValidAccessToken`/`getShopId`/`listActiveTenantIds` (mesma conexão já validada), mesmo padrão de backfill (`create_time`) vs incremental (`update_time`) do Mercado Livre.
- Wiring: registrado/exportado em `marketplace-intelligence.module.ts`, injetado em `ORDER_CAPABLE_PROVIDERS` (`orders.module.ts`) — nenhuma linha de `OrderProviderRegistry`/`OrderSyncOrchestrator` mudou. Seed de `ProviderSyncSchedule` (`SHOPEE_ORDERS`, 10 min) adicionado a `prisma/seed.ts` — mesmo bug de produção já corrigido para Mercado Livre/Nuvemshop (sem essa linha, o cron nunca sincroniza o canal).

**AVISO DE HONESTIDADE:** diferente da camada de conexão (auth_partner/token/get_shop_info — já confirmada num handshake real), a captura de pedidos em si nunca foi exercitada contra a Shopee de verdade — segue a documentação pública do Shopee Open Platform v2. Detalhe completo (as duas diferenças estruturais da Shopee — janela de 15 dias e listagem/detalhe separados) em `docs/orders-architecture.md`, seção 16.

Testes: `shopee-order-status.mapper.spec.ts`, `shopee-order-normalizer.spec.ts`, `shopee-order.provider.spec.ts` — mesma cobertura de `mercado-livre-order.provider.spec.ts` (capacidade declarada, gate de shopId não resolvido, backfill vs incremental, propagação de exceção sem conexão ativa).
