...
**AVISO DE HONESTIDADE, reforçado (mesmo padrão de toda integração nova deste projeto, mais forte aqui que no Mercado Livre):** o fluxo inteiro foi montado a partir da documentação pública do Shopee Open Platform e de um guia de terceiro — nunca exercitado contra uma chamada real (app em status "Developing", só credenciais de teste, sandbox de desenvolvimento sem rede real). Três pontos especificamente não confirmados, cada um flagado no código: (1) se a base de teste usa `https://partner.shopeemobile.com` ou um subdomínio de sandbox dedicado — `SHOPEE_BASE_URL` permite sobrepor sem alterar código; (2) se a técnica de embutir `state` no `redirect` sobrevive ao redirect real da Shopee; (3) o formato exato da resposta de erro de negócio em cenários de `code` inválido/expirado. O primeiro handshake real do usuário (clicar em "Conectar Shopee" de verdade) é quem confirma ou refuta cada um desses pontos — ver docs/auth-security.md, seção 9.
...
**Pendente, deliberadamente fora desta sprint:** só depois do handshake real do usuário confirmar os três pontos do aviso de honestidade acima (clicando em "Conectar Shopee"/"Testar conexão" na tela de Integrações), o `ShopeeOrderProvider` (pedidos) propriamente dito.
