    ...
    <div className="relative inline-block">
      <button
        ...
        className={[
          ...
          topSeverity === 'OPORTUNIDADE' ? 'bg-neon/15 text-foreground hover:animate-neonPulse' : meta.badgeClass,
        ].join(' ')}
      ...
      {open && (
        <div className="absolute right-0 z-20 mt-2 w-72 rounded-xl border border-border bg-card p-3 text-left shadow-card">
          <p className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Sugestões de IA</p>
            ...
            {insights.map((insight) => {
              ...
              return (
                <li key={insight.id} className="flex items-start gap-2">
                  ...
                  <span className="text-xs text-foreground">{insight.message}</span>
                </li>
              );
            })}
          </ul>
